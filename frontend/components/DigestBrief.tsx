 ...
 *
 * The reference's stats row also shows an "acknowledged" count — dropped
 * here, since the digest only ever returns currently-unacked flags, so
 * there's no real number to back it without a backend change. */
export function DigestBrief({ brief, flags }: { brief: string | null; flags: Flag[] | null }) {
