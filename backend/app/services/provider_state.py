...
async def load_provider_state(pool: asyncpg.Pool) -> asyncpg.Record | None:
    return await pool.fetchrow(
        "SELECT mode, frozen_at, last_successful_fetch, updated_at FROM provider_state WHERE id = 1"
    )
...
async def save_provider_state(
    ...
    await pool.execute(
        ...
        SET mode = EXCLUDED.mode, frozen_at = EXCLUDED.frozen_at,
            last_successful_fetch = EXCLUDED.last_successful_fetch, updated_at = EXCLUDED.updated_at
        """,
        mode, frozen_at, last_successful_fetch,
    )
