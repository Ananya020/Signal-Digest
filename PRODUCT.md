...
`signal_type = 'volatility_regime'` (stdev_5d/stdev_30d ratio crossing 1.5) is still computed and persisted by the scoring engine exactly as designed — nothing about its computation changed. Live testing (Phase 5/6) surfaced a real UX problem: with the original static baselines (one baseline row per ticker, not recomputed per replay day — since superseded, see below), the same handful of tickers whose end-of-window stdev ratio happens to cross the trigger flag on *every single replay cycle*, with the same generic explanation text each time. That's repetitive noise, not a meaningful "something changed" signal.

**Update (Workstream 1, 2026-09-05 — baselines are no longer static.)** Baselines now roll forward per trading day, look-ahead-safe (see PROGRESS.md, DATA_MODEL.md). The premise that motivated hiding `volatility_regime` — a fixed baseline producing the same trigger on the same tickers every cycle — no longer holds by construction: a 20-cycle replay run against real seeded data showed the flagged-ticker set genuinely rotating day to day (a `RELIANCE.NS`/`ICICIBANK.NS`/`WIPRO.NS`/`NESTLEIND.NS` cluster for several consecutive days, then a shift to `NTPC.NS`/`POWERGRID.NS`, then `MARUTI.NS`/`ADANIENT.NS`/`ASIANPAINT.NS`). Digest surfacing is still deliberately **not** re-enabled in this workstream — the display decision needs more observation (does the explanation text stay meaningfully distinct per day, does the trigger threshold still make sense against a smaller rolling sample_size on early days) — but the original technical justification for hiding it is gone.

Resolution (unchanged): `GET /watchlists/{id}/digest` filters to `signal_type = 'price_zscore'` only (`backend/app/services/digest.py`). No rows are deleted, no scoring logic touched — this remains purely a display decision, one line to reverse whenever that separate call is made. The digest is also explicitly ranked by `|z_score|` descending, most statistically unusual first, now that price_zscore is the digest's only signal type.
